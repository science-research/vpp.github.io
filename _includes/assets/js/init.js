    ...
    $(function() {
        ...
        function screenshotSlider() {
            ...
            $("#screenshots-carousel").owlCarousel({
                nav: true,
                navText: ["<span class='nav-button nav-prev'></span>",
                    "<span class='nav-button nav-next'></span>"
                ],
                autoplay: false,
                center: false,
                lazyLoad: true,
        ...
        $(".nav-wrapper #cart").on("click", function() {
            ...
            $(".cart-items li i").each(function() {
                $(this).on("click", function() {
                    ...
                    var qty = parseInt($(this).first().parent().parent().find(
                        ".item-qty").attr("title"));
                    var price = parseInt($(this).first().parent().parent().find(
                        ".item-price").attr("title"));
                    ...
                    for (var i = 0; i < products.length; i++) {
                        if (selectedSku === products[i].sku) {
                            ...
                            $("#item-num-mobile").text(currentItemNum - products[i]
                                .qty);
                            break;
                        }
                    }
        ...
        $('body').click(function(e) {
            ...
            if (!$(e.target).closest('.shopping-cart').length && !$(e.target).closest('a#cart')
                .length && !$(e.target).hasClass('remove-item')) {
                $('.shopping-cart').hide();
            }
        });
            ...
            if (products.length) {
                for (var i = 0; i < products.length; i++) {
                    ...
                    $(".cart-items").add('<li class="clearfix">\n' +
                        ...
                        '                    <div class="item-info">\n' +
                        '                        <span class="item-name">' + products[i].name +
                        '<i id="' + products[i].sku +
                        '" class="material-icons remove-item">clear</i></span>\n' +
                        '                        <span class="item-qty" title="' + products[i].qty +
                        '">' + products[i].qty + ' X </span>\n' +
                        '                        <span class="item-price" title ="' + products[i]
                        .price + '">$' + products[i].price + '</span>\n' +
                        '\n' +
                        '                    </div>\n' +
                ...
                $('<div class="shopping-cart-footer clearfix">\n' +
                    ...
                    '                <span class="price" title="' + totalPrice + '">$' + totalPrice +
                    '</span>\n' +
                    '            </div>\n' +
        ...
        function getAddToCartUrl(selector, currentHref) {
            ...
            var additionalUrl = "adds?" + url + "submit=Checkout+now&mprc=" + selector.find(
                "input[name=mprc]").val();
        ...
        function getAddToCartProductInfo(selector) {
            if (localStorage) {
                ...
                if (selector.find("input[type=checkbox]").is(":checked")) {
                    price = (productPrice / selector.find("input[type=number]").val()) - selector.find(
                        "input[type=checkbox]").attr("title");
                    isInstallation = true;
                } else {
        ...
        function getExtensionsInfo(products, price) {
            ...
            for (var id in products) {
                if (products[id].sku === selector.find(".sku").attr("value")) {
                    products[id].qty = products[id].qty + parseInt(selector.find("input[type=number]")
                    .val());
                    isDuplicated = true;
                    break;
                } else {
        ...
        function getInstallationServiceInfo(products, isInstallation) {
            ...
            if (isInstallationDuplicated === false && isInstallation === true) {
                products.push({
                    ...
                    logo: selector.find(".installation-logo").attr("value") +
                        '/assets/img/extensions/installation-service.png',
                    qty: parseInt(selector.find("input[type=number]").val()),
                    price: 50
                });
            }
